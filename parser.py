...
class Document:
    ...
    def split_pages(self):
        ...
        for i in range(len(doc)):
            ...
            page = doc.loadPage(i)
            pixmap = page.getPixmap(matrix=fitz.Matrix(zoom_factor, zoom_factor))

            # Initialize bytes buffer and write PNG image to buffer
            buffer = BytesIO()
            buffer.write(pixmap.getPNGData())
            buffer.seek(0)
...
class Page:
    ...
    def read_cells(self):
        ...
        2. Extracts coordinates of bounding rectangle for each contour
        ...
        df['xywh'] = df['contours'].apply(cv2.boundingRect)
